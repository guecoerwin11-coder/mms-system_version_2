...
export default function StudentProfile() {
  ...
  const [form, setForm] = useState({ name: user?.name || '', year: user?.year || '', section: user?.section || '', gmail: user?.gmail || '' });
  ...
  const handleForgot = async () => {
    try { await authAPI.forgotPassword({ email: user.email }); toast.success('Reset link sent to your Gmail!'); }
  ...
  return (
    <div>
      ...
      <p className="page-sub">Edit your information · Change photo · Reset password</p>
      <div className="grid2">
        <div className="card">
          ...
          <div className="btn-row">
            <button className="btn btn-wine" onClick={handleSave} disabled={saving}>{saving ? 'Saving...' : 'Save Changes'}</button>
          </div>
        </div>
        <div className="card">
          <div className="card-title">Change Password</div>
          {[['Current Password','currentPassword'],['New Password','newPassword'],['Confirm Password','confirm']].map(([label, key]) => (
            <div className="form-group" key={key}>
              <label className="form-label">{label}</label>
          ...
          <div className="btn-row">
            <button className="btn btn-wine" onClick={handlePw} disabled={pwSaving}>{pwSaving ? 'Updating...' : 'Update Password'}</button>
            <button className="btn btn-outline" onClick={handleForgot}>Reset via Email</button>
          </div>
        </div>
      </div>
    </div>
